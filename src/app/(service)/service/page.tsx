...
export default function ServicePage() {
    ...
    const triggerPhotoSelect = (type: 'BROKEN_DEVICE' | 'BARCODE' | 'WORKING_DEVICE' | 'OTHER') => {
        ...
        <div style={{ maxWidth: '600px', margin: '0 auto', paddingBottom: '80px' }}>
            ...
            {showActionModal && actionRecord && (
                <div className="modal-overlay" onClick={() => setShowActionModal(false)}>
                    <div className="modal" style={{ maxHeight: '92vh', overflowY: 'auto', maxWidth: '820px', width: '95vw', padding: 0 }} onClick={(e) => e.stopPropagation()}>
                        {/* Compact Modal Header */}
                        <div className="modal-header" style={{ padding: '8px 14px', borderBottom: '1px solid var(--border-primary)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <span style={{ fontWeight: 700, fontSize: '14px' }}>
                                    {actionRecord.type === 'PICKUP' ? '📥 Teslim Al' : '📤 Teslim Et'}
                                </span>
                                <span style={{ fontSize: '11px', color: 'var(--brand-primary)', fontFamily: 'monospace', fontWeight: 700, background: 'rgba(59, 130, 246, 0.1)', padding: '1px 6px', borderRadius: '4px' }}>
                                    {(actionRecord.ticket as any).ticketNo}
                                </span>
                            </div>
                            <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowActionModal(false)} style={{ padding: '1px 5px' }}>✕</button>
                        </div>

                        {/* Modal Body - 2 Column Grid */}
                        <div className="modal-body" style={{ padding: '10px 14px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '10px', alignItems: 'start' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                ...
                                <div style={{ padding: '8px 10px', background: 'var(--bg-secondary)', borderRadius: '8px', border: '1px solid var(--border-primary)' }}>
                                    ...
                                    {showAddOpForm && (
                                        <div style={{ padding: '6px', background: 'var(--bg-primary)', border: '1px solid var(--brand-primary)', borderRadius: '6px', marginBottom: '6px' }}>
                                            <div style={{ fontSize: '10.5px', fontWeight: 600, marginBottom: '4px', color: 'var(--brand-primary)' }}>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                                <div style={{ display: 'flex', gap: '4px' }}>
                                                    <select
                                                        ...
                                                        style={{ flex: 1, fontSize: '11px', padding: '2px 20px 2px 4px', minHeight: '26px', height: '26px' }}
                                                    >
                                                    ...
                                                    <input
                                                        ...
                                                        style={{ width: '75px', fontSize: '10.5px', padding: '2px 4px', height: '26px' }}
                                                    />
                                                </div>
                                                {newOpType === 'OTHER' && (
                                                    <input
                                                        ...
                                                        style={{ fontSize: '10.5px', padding: '2px 4px', height: '26px' }}
                                                    />
                                                )}
                                    ...
                                    <div style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>
                                        ...
                                        {(actionRecord.ticket as any).accessories?.length > 0 && (
                                            <div style={{ marginTop: '2px', paddingTop: '2px', borderTop: '1px dashed var(--border-primary)' }}>
                                                {(actionRecord.ticket as any).accessories.map((acc: any) => (
                                                    <div key={acc.id} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1px', color: 'var(--text-secondary)' }}>
                                        ...
                                        {getNewAccessoryTotal() > 0 && (
                                            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '1px', color: 'var(--brand-primary)', fontWeight: 600 }}>
                                                <span>+ Yeni Satış ({accQty}x):</span>
                                                <span>{formatCurrency(getNewAccessoryTotal())}</span>
                                            </div>
                                        )}

                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '3px', paddingTop: '3px', borderTop: '1px solid var(--border-primary)', fontWeight: 600, color: 'var(--text-primary)' }}>
                                            <span>Toplam:</span>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '3px', paddingTop: '3px', borderTop: '1px solid var(--border-primary)', fontWeight: 700, color: getRemaining() > 0 ? 'var(--color-danger)' : 'var(--color-success)' }}>
                                            <span style={{ fontSize: '11.5px' }}>Kalan Ödeme:</span>
                                            <span style={{ fontSize: '12.5px' }}>{formatCurrency(getRemaining())}</span>
                                        </div>
                                    </div>
                                </div>

                                {/* Compact Photo Capture Section */}
                                <div style={{ padding: '6px 8px', background: 'var(--bg-secondary)', borderRadius: '8px', border: '1px solid var(--border-primary)' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                                        <span style={{ fontSize: '11px', fontWeight: 600 }}>
                                            📷 Fotoğraf Ekle <span style={{ color: 'var(--color-danger)' }}>*</span>
                                        </span>
                                        {photos.length > 0 && (
                                    ...
                                    {/* Single Hidden Input for Native OS Chooser (Camera / Gallery / Files) */}
                                    <input ref={photoInputRef} type="file" accept="image/*" multiple style={{ display: 'none' }} onChange={handleFileCapture} />

                                    {/* Action Buttons for Pickup vs Delivery */}
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                        {actionRecord.type === 'PICKUP' ? (
                                            <>
                                                {/* Arızalı Cihaz */}
                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--bg-tertiary)', padding: '4px 8px', borderRadius: '5px' }}>
                                                    <span style={{ fontSize: '11px', fontWeight: 600 }}>📸 Arızalı Cihaz</span>
                                                    <button
                                                        type="button"
                                                        className="btn btn-primary btn-xs"
                                                        onClick={() => triggerPhotoSelect('BROKEN_DEVICE')}
                                                ...
                                                {/* Barkod */}
                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--bg-tertiary)', padding: '4px 8px', borderRadius: '5px' }}>
                                                    <span style={{ fontSize: '11px', fontWeight: 600 }}>🔖 Barkod</span>
                                                    <button
                                                        type="button"
                                                        className="btn btn-primary btn-xs"
                                                        onClick={() => triggerPhotoSelect('BARCODE')}
                                                        style={{ fontSize: '10.5px', padding: '2px 8px', display: 'inline-flex', alignItems: 'center', gap: '3px' }}
                                                        title="Barkod Fotoğrafı Seç veya Çek"
                                                    >
                                                        📷 Fotoğraf Seç / Çek
                                                    </button>
                                                </div>
                                            ...
                                            /* Çalışır Hali (Delivery) */
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--bg-tertiary)', padding: '4px 8px', borderRadius: '5px' }}>
                                                <span style={{ fontSize: '11px', fontWeight: 600 }}>✅ Çalışır Hali</span>
                                                <button
                                                    type="button"
                                                    className="btn btn-primary btn-xs"
                                                    onClick={() => triggerPhotoSelect('WORKING_DEVICE')}
                                                    style={{ fontSize: '10.5px', padding: '2px 8px', display: 'inline-flex', alignItems: 'center', gap: '3px' }}
                                                    title="Çalışır Hali Fotoğrafı Seç veya Çek"
                                                >
                                                    📷 Fotoğraf Seç / Çek
                                                </button>
                                            </div>
                                        ...
                                        {/* Diğer Fotoğraf */}
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--bg-tertiary)', padding: '4px 8px', borderRadius: '5px' }}>
                                            <span style={{ fontSize: '11px', fontWeight: 600 }}>📷 Diğer Ek Foto</span>
                                            <button
                                                type="button"
                                                className="btn btn-ghost btn-xs"
                                                onClick={() => triggerPhotoSelect('OTHER')}
                                                style={{ fontSize: '10.5px', padding: '2px 8px', border: '1px solid var(--border-primary)', display: 'inline-flex', alignItems: 'center', gap: '3px' }}
                                                title="Ek Fotoğraf Seç veya Çek"
                                            >
                                                ➕ Fotoğraf Ekle
                                            </button>
                                        </div>
                                    ...
                                    {photos.length > 0 && (
                                        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginTop: '6px', paddingTop: '4px', borderTop: '1px dashed var(--border-primary)' }}>
                                            {photos.map((p, i) => (
                                                <div key={i} style={{ position: 'relative', width: '42px', height: '42px' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                ...
                                <div style={{ padding: '6px 8px', background: 'rgba(245, 158, 11, 0.08)', border: '1px solid rgba(245, 158, 11, 0.25)', borderRadius: '8px' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2px' }}>
                                        <span style={{ fontSize: '10.5px', color: '#d97706', fontWeight: 700 }}>🔍 Cihaz Durumu / Hasar</span>
                                        {!isEditingCondition ? (
                                            <button
                                                type="button"
                                                className="btn btn-ghost btn-xs"
                                                onClick={() => setIsEditingCondition(true)}
                                                style={{ fontSize: '10px', color: '#d97706', padding: '1px 4px' }}
                                            >
                                            ...
                                            <button
                                                ...
                                                style={{ fontSize: '10px', padding: '1px 5px', background: '#d97706', borderColor: '#d97706' }}
                                            >
                                                💾 Kaydet
                                            </button>
                                        )}
                                    ...
                                    {isEditingCondition ? (
                                        <input
                                            ...
                                            placeholder="Örn: Arka kapak kırık..."
                                            style={{ fontSize: '11px', padding: '2px 6px', width: '100%', height: '26px', marginTop: '2px' }}
                                            autoFocus
                                        />
                                    ) : (
                                        <div style={{ fontSize: '11px', color: 'var(--text-primary)', fontWeight: 500, lineHeight: 1.2 }}>
                                ...
                                <div style={{ background: 'var(--bg-secondary)', padding: '6px 8px', borderRadius: '8px', border: '1px solid var(--border-primary)' }}>
                                    <label style={{ display: 'flex', alignItems: 'center', gap: '6px', width: '100%', cursor: 'pointer', marginBottom: addAccessoryEnabled ? '4px' : 0 }}>
                                        <input type="checkbox" checked={addAccessoryEnabled} onChange={(e) => setAddAccessoryEnabled(e.target.checked)} />
                                        <span style={{ fontWeight: 600, fontSize: '11px' }}>🛍 Aksesuar Satışı</span>
                                    </label>
                                    {addAccessoryEnabled && (
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '3px', marginTop: '3px' }}>
                                            ...
                                            {accProduct && (
                                                <div style={{ display: 'flex', gap: '4px' }}>
                                                    <input type="number" className="form-input" placeholder="Adet" value={accQty} onChange={(e) => setAccQty(e.target.value)} min="1" style={{ width: '60px', fontSize: '10.5px', padding: '2px 4px', height: '26px' }} />
                                                    <input type="number" className="form-input" placeholder="Birim Fiyat ₺" value={accPrice} onChange={(e) => setAccPrice(e.target.value)} min="0" style={{ flex: 1, fontSize: '10.5px', padding: '2px 4px', height: '26px' }} />
                                                </div>
                                            )}
                                        </div>
                                    )}
                                ...
                                {actionRecord.type === 'DELIVERY' && (
                                    <div style={{ background: 'var(--bg-secondary)', padding: '6px 8px', borderRadius: '8px', borderLeft: '3px solid var(--brand-primary)', border: '1px solid var(--border-primary)' }}>
                                        <div style={{ fontSize: '11px', fontWeight: 600, marginBottom: '4px' }}>
                                            💰 Ödeme Girişi {getRemaining() > 0 ? '(Zorunlu)' : ''}
                                        </div>

                                        {paymentsList.map((p, idx) => (
                                             <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--bg-primary)', padding: '2px 5px', borderRadius: '4px', marginBottom: '3px', fontSize: '11px' }}>
                                                 <div>
                                                 ...
                                                 <button type="button" className="btn btn-ghost btn-xs" style={{ color: 'var(--color-danger)', border: 'none', padding: '0 3px' }} onClick={() => setPaymentsList(prev => prev.filter((_, i) => i !== idx))}>✕</button>
                                             </div>
                                        ))}
                                        ...
                                        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                                            ...
                                            <select
                                                ...
                                                style={{ width: '95px', fontSize: '10.5px', padding: '2px 18px 2px 4px', minHeight: '26px', height: '26px' }}
                                            >
                                            ...
                                            <button
                                                ...
                                                style={{ fontSize: '10.5px', padding: '2px 6px', height: '26px' }}
                                            >
                                                Ekle
                                            </button>
                                        </div>
                                        ...
                                        {(payMethod === 'BANK_TRANSFER' || payMethod === 'CREDIT_CARD') && (
                                            <div style={{ marginTop: '3px' }}>
                                                <select
                                                    ...
                                                    style={{ fontSize: '10.5px', padding: '2px 20px 2px 4px', minHeight: '26px', height: '26px', width: '100%' }}
                                                >
                                        ...
                                        {payMethod === 'BANK_TRANSFER' && (
                                            <input
                                                ...
                                                style={{ marginTop: '3px', fontSize: '10.5px', padding: '2px 4px', height: '26px' }}
                                            />
                                        )}
                                    </div>
                                )}
                                ...
                                <div>
                                    <textarea
                                        ...
                                        rows={1}
                                        style={{ fontSize: '11px', padding: '4px 6px', resize: 'none', width: '100%', height: '36px', minHeight: '36px' }}
                                    />
                                    {actionError && <div style={{ color: 'var(--color-danger)', fontSize: '10.5px', marginTop: '1px' }}>{actionError}</div>}
                                </div>
                            </div>
                        </div>

                        {/* Compact Modal Footer */}
                        <div className="modal-footer" style={{ padding: '8px 14px', borderTop: '1px solid var(--border-primary)', display: 'flex', justifyContent: 'flex-end', gap: '6px' }}>
                            <button className="btn btn-secondary btn-sm" onClick={() => setShowActionModal(false)} style={{ fontSize: '11.5px', padding: '4px 12px' }}>İptal</button>
                            <button className="btn btn-success btn-sm" disabled={savingAction} onClick={handleCompleteService} style={{ fontSize: '11.5px', padding: '4px 14px', fontWeight: 600 }}>
