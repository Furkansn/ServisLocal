...
export const TURKEY_LOCATIONS: CityData[] = [
    {
        name: 'Adana',
...
export const CITIES_LIST = TURKEY_LOCATIONS.map(c => c.name);

export function getDistrictsByCity(cityName: string): string[] {
    if (!cityName) return [];
    const found = TURKEY_LOCATIONS.find(
        c => c.name.toLocaleLowerCase('tr-TR') === cityName.trim().toLocaleLowerCase('tr-TR')
    );
